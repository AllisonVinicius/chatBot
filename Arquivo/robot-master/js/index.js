...
function send_message(conv,message){
	...
	$("#converse").html(conv +"<span class = 'current-msg'>" + "<span id='chat-bot'>robot: </span>" + message + "</span>");
	$(".current-msg").hide();
...
function get_username(conv){
	send_message(conv,"Ola, como e seu nome?");
}

function ai(conv,message){
	if (username<4) {
		username = message;
		send_message(conv,"ola, "+ username + ", como voce esta?");
	}
	else{
		// $("#send").click(function(){
...
$(function(){
	...
	$("#send").click(function(){
		...
		if (usermsg != "") {
			...
			$("#converse").html(conv + "<span id='chat-user'> Voce: </span>" + usermsg);
